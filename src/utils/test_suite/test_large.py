...
mpc_horion_list = [5, 10, 20]


@log_time(time_in_mins=True)
def both_directions_all_orientations_all_horizons(
    model: ConfigWAM,
    ...
    data = {}
    for mpc_horizon in mpc_horion_list:
        mpc_horizon_data = []
        ...
        data[mpc_horizon] = mpc_horizon_data
    for mpc_horizon in mpc_horion_list:
        results_df = pd.DataFrame(data[mpc_horizon]).sort_values(
            by=["direction", "radius"]
        )
        print(
            table_header(
